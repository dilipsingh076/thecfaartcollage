 ...
 * @returns Menu data
 */
export const getMenuData = async (useCache: boolean = true): Promise<MenuData> => {
  const cacheKey = 'menu-data';
  
  // Try to get data from cache if caching is enabled
  if (useCache) {
    const cachedData = getFromCache<MenuData>(cacheKey, API_CACHE_CONFIG.menuData);
  ...
  // Fetch data from API
  const response = await fetchApi<MenuData>(API_ENDPOINTS.MENUS);
 ...
 */
export const clearMenuDataCache = (): void => {
  cache.delete('menu-data');
}; 